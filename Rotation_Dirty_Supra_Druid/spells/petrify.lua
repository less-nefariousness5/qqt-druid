...
local petrify_menu_elements = 
{
    ...
    main_boolean       = checkbox:new(true, get_hash(my_utility.plugin_label .. "petrify_disable_enable_ability")),
    targeting_mode     = combo_box:new(3, get_hash(my_utility.plugin_label .. "petrify_base_targeting_mode")),
    cast_range         = slider_int:new(1, 35, 15, get_hash(my_utility.plugin_label .. "petrify_cast_range")),
}
...
local function menu()                                                                                    
    if petrify_menu_elements.main_tab:push("Petrify")then
        ...
        if petrify_menu_elements.main_boolean:get() then
            petrify_menu_elements.targeting_mode:render("Targeting Mode", my_utility.targeting_modes,
                my_utility.targeting_mode_description)
            petrify_menu_elements.cast_range:render("Cast Range", "Range in yards to cast Petrify")
        end
...
local function logics(target)
    ...
    local cast_range = petrify_menu_elements.cast_range:get()
    ...
    if cast_spell.target(target, spell_data.petrify.spell_id, 0, false) then
        local current_time = get_time_since_inject();
        next_time_allowed_cast = current_time + my_utility.spell_delays.regular_cast;

        console.print("Cast Petrify - Target: " ..
            my_utility.targeting_modes[petrify_menu_elements.targeting_mode:get() + 1] .. 
            " - Range: " .. cast_range);
        return true;
    end;
