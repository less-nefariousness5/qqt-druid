...
local menu_elements_sorc_base_lightning = 
{
    ...
    priority_target       = checkbox:new(false, get_hash(my_utility.plugin_label .. "chain_lightning_priority_target_bool")),
    elite_only            = checkbox:new(false, get_hash(my_utility.plugin_label .. "chain_lightning_elite_only_bool")),
}
...
local function menu()
    ...
    if menu_elements_sorc_base_lightning.tree_tab:push("Chain Lightning") then
        ...
        if menu_elements_sorc_base_lightning.main_boolean:get() then
            menu_elements_sorc_base_lightning.priority_target:render("Priority Target", "Target priority: Boss > Champion > Elite > Normal")
            menu_elements_sorc_base_lightning.elite_only:render("Elite Only", "Only attack elite, champion and boss monsters")
        end
...
local function get_priority_target(target_selector_data)
    ...
    local elite_only = menu_elements_sorc_base_lightning.elite_only:get()
...
local function logics(best_target, target_selector_data)
    ...
    -- Default enable_chain_lightning to true if priority target is not active
    local enable_chain_lightning = not menu_elements_sorc_base_lightning.priority_target:get();
    
    -- Priority Targeting Mode: prioritize targets by type
    if menu_elements_sorc_base_lightning.priority_target:get() and target_selector_data then
        local priority_best_target, target_type = get_priority_target(target_selector_data)
        
        if priority_best_target then
            if cast_spell.target(priority_best_target, chain_lightning_spell_data, false) then
                local current_time = get_time_since_inject()
                next_time_allowed_cast = current_time + 0.8
                if debug_enabled then console.print("[PRIORITY] Chain Lightning cast at " .. target_type .. " target") end
                return true
            end
        else
            if debug_enabled then console.print("[PRIORITY] No valid priority target found for Chain Lightning") end
        end
    ...
    if cast_spell.target(best_target, chain_lightning_spell_data, false) then

        local current_time = get_time_since_inject();
        next_time_allowed_cast = current_time + 0.8;
        
        if debug_enabled then console.print("Sorcerer Plugin, Casted Chain Lightning"); end
        return true;
    end;
