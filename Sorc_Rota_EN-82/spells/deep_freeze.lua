...
local function logics(best_target, target_selector_data)
    ...
    if menu_elements_deep_freeze.priority_target:get() and target_selector_data then
        ...
        if not has_priority then
            if debug_enabled then console.print("[PRIORITY] No priority targets nearby for Deep Freeze") end
            return false
        else
            if debug_enabled then console.print("[PRIORITY] " .. target_type .. " target detected, proceeding with Deep Freeze") end
        end
    end
