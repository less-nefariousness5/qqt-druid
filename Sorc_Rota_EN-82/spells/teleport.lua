...
local function cast_for_exploration_movement()
    ...
    if cast_spell.position(spell_id_tp, target_position, 0.3) then
        ...
        if debug_enabled then console.print("[Exploration Mode] Teleport Movement Cast") end
        return true;
    end
...
local function logics(entity_list, target_selector_data, best_target)
    ...
    if menu_elements_sorc_base.tele_gtfo:get() then
        ...
        if health_percentage < 0.90 then
            ...
            if debug_enabled then console.print("Sorcerer Plugin, Casted Teleport due to I need to GTFO"); end
            return true;
        end
    end
    ...
    if menu_elements_sorc_base.teleport_on_self:get() then
        ...
        if debug_enabled then console.print("Sorcerer Plugin, Casted Teleport on Self"); end
        return true;
    end
        ...
        if best_target then
            ...
            if safe_position then
                if cast_spell.position(spell_id_tp, safe_position, 0.3) then
                    ...
                    if debug_enabled then console.print("Sorcerer Plugin, Safe Teleport to " .. target_type .. " [" .. teleport_mode .. "]") end
                    return true
                end
            else
                if debug_enabled then console.print("No safe teleport position found for " .. target_type) end
            end
        else
            if debug_enabled then console.print("No valid priority target found for Teleport") end
        end
    end
    ...
    if not safe_position then
        if debug_enabled then console.print("No safe teleport position available") end
        return false;
    end
    ...
    if debug_enabled then console.print("Sorcerer Plugin, Safe Teleport [" .. teleport_mode .. "]"); end
    return true;
