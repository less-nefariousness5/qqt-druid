...
on_render_menu(function ()
    ...
    -- Add Mighty Throw notifications toggle
    menu.mighty_throw_notifications:render("Show Mighty Throw Notifications", "Display on-screen timing notifications")
...
-- Mighty Throw rotation state
local mighty_throw_rotation = {
    last_empowered_cast = 0,
    is_in_rotation = false,
    rotation_start_time = 0,
    boss_mode = false,
    notification_text = "",
    notification_color = color_white(255),
    notification_time = 0
}

local function show_screen_notification(text, color, duration)
    if not menu.mighty_throw_notifications:get() then
        return
    end
    
    mighty_throw_rotation.notification_text = text
    mighty_throw_rotation.notification_color = color or color_white(255)
    mighty_throw_rotation.notification_time = get_time_since_inject() + (duration or 2.0)
end

local function execute_mighty_throw_rotation(current_time, best_target, is_boss, entity_list, target_selector_data)
    if not spells.mighty_throw.menu_elements.enable_combo_rotation:get() then
        return false
    end
    
    mighty_throw_rotation.is_in_rotation = true
    mighty_throw_rotation.rotation_start_time = current_time
    mighty_throw_rotation.boss_mode = is_boss
    
    local mode_text = is_boss and "BOSS MODE" or "REGULAR MODE"
    show_screen_notification("⚡ MIGHTY THROW ROTATION - " .. mode_text, color_yellow(255), 1.5)
    
    local sequence_delay = 0.1
    local step = 1
    
    -- Step 1: Call of the Ancients (immediately)
    if spells.call_of_the_ancients.logics() then
        show_screen_notification("1/5: Call of the Ancients", color_green(200), 0.8)
        step = step + 1
    end
    
    -- Wait for next frame then continue sequence
    cast_end_time = current_time + 0.05
    
    -- Step 2: Hammer of Ancients (only for regular enemies)
    if not is_boss and step <= 5 then
        if spells.hammer_of_ancients.logics(best_target) then
            show_screen_notification(step .. "/5: Hammer of Ancients", color_green(200), 0.8)
            step = step + 1
        end
    end
    
    -- Step 3: Ground Stomp
    if step <= 5 and spells.ground_stomp.logics(best_target) then
        show_screen_notification(step .. "/5: Ground Stomp", color_green(200), 0.8)
        step = step + 1
    end
    
    -- Step 4: Shouts
    if step <= 5 and spells.rallying_cry.logics() then
        show_screen_notification(step .. "/5: Rallying Cry", color_green(200), 0.8)
        step = step + 1
    end
    
    if step <= 5 and spells.war_cry.logics() then
        show_screen_notification(step .. "/5: War Cry", color_green(200), 0.8)
        step = step + 1
    end
    
    -- Step 5: Mighty Throw (final)
    if step <= 5 and spells.mighty_throw.logics(best_target, entity_list, target_selector_data) then
        show_screen_notification("5/5: MIGHTY THROW!", color_red(255), 1.2)
    end
    
    mighty_throw_rotation.last_empowered_cast = current_time
    mighty_throw_rotation.is_in_rotation = false
...
on_update(function ()
    ...
    -- MIGHTY THROW ROTATION - HIGHEST PRIORITY
    if spells.mighty_throw.menu_elements.enable_combo_rotation:get() then
        local empowering_stacks, is_empowered = spells.mighty_throw.check_mighty_throw_buffs()
        local is_boss = spells.mighty_throw.is_boss_nearby(target_selector_data)
        
        -- Only execute rotation when empowered buff is active
        if is_empowered and not mighty_throw_rotation.is_in_rotation then
            if execute_mighty_throw_rotation(current_time, best_target, is_boss, entity_list, target_selector_data) then
                cast_end_time = current_time + 1.0 -- Block other actions for 1 second
                return
            end
        end
        
        -- Block all other actions during rotation
        if mighty_throw_rotation.is_in_rotation then
            return
        end
        
        -- On-screen warning system
        if empowering_stacks >= 10 then
            local stacks_remaining = 12 - empowering_stacks
            show_screen_notification("⚠️ MIGHTY THROW READY IN " .. stacks_remaining .. " STACKS", 
                                   color_orange(255), 1.0)
        elseif empowering_stacks >= 8 then
            show_screen_notification("⚡ Mighty Throw Building: " .. empowering_stacks .. "/12", 
                                   color_blue(200), 0.5)
        end
    end
...
--#region Logic calls
    ...
    if spells.mighty_throw.logics(best_target, entity_list, target_selector_data) then
        cast_end_time = current_time + 0.3;
        return;
    end;
...
on_render(function ()
 ...
 -- console.print(max_range)
    ...
    -- Render Mighty Throw notifications
    local current_time = get_time_since_inject()
    if current_time < mighty_throw_rotation.notification_time and 
       mighty_throw_rotation.notification_text ~= "" then
        
        local screen_center_x = get_screen_width() / 2
        local screen_top_y = get_screen_height() * 0.20 -- 20% from top
        
        -- Create position vector
        local text_pos = vec2:new(screen_center_x, screen_top_y)
        
        -- Render the notification text
        graphics.text_2d(mighty_throw_rotation.notification_text, 
                        text_pos, 
                        24, -- font size
                        mighty_throw_rotation.notification_color)
    end
